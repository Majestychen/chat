...
docker run --rm -it -d -v /root/chat:/root/.pm2/logs/ -p 3005:3000 --name chat --link db:db chat

echo "[chat] start node server ..."
docker exec -it chat /bin/bash -c "NODE_ENV=product pm2 start /root/chat/server.js" 
